...
const fileSystemApi = {
  key: "vibeos-files",
  positionKey: "vibeos-desktop-file-positions",
...
const desktopIconApi = {
  key: "vibeos-desktop-icon-positions",
  icons: {},
  drag: null,
  ignoreClickUntil: 0,
  defaults: {
...
const terminalApi = {
  ...
  printCommand(command) {
    this.print(`guest@vibeos:${this.cwd}$ ${command}`, "command");
  },
  ...
  updatePrompt() {
    if (this.prompt) {
      this.prompt.textContent = `guest@vibeos:${this.cwd}$`;
    }
  },

  bind() {
    this.print("VibeOS Terminal");
...
const systemApi = {
  ...
  async measureNetworkProbe() {
    ...
    try {
      await fetch(`https://www.gstatic.com/generate_204?vibeos=${Date.now()}`, {
        cache: "no-store",
        mode: "no-cors",
...
const notificationApi = {
  ...
  show({ title = "VibeOS", message, actionLabel = "", onAction = null }) {
  ...
  maybeWelcome() {
    ...
    window.setTimeout(() => {
      notificationApi.show({
        title: "Welcome",
        message: "Hey new guy, welcome to VibeOS. You should check the guide to use vibeos to its max!",
